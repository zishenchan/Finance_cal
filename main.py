...
def calculate_direct_method():
    ...
    # change into the monthly deposit * 12 + initial deposit ) * before total year to compound  
...
# add a label for month deposit

tk.Label(frame, text="Monthly Deposit:").grid(row=1, column=0)
initial_deposit_entry = tk.Entry(frame)
initial_deposit_entry.grid(row=1, column=1)


tk.Label(frame, text="Years to Compound:").grid(row=2, column=0)
years_to_compound_entry = tk.Entry(frame)
years_to_compound_entry.grid(row=2, column=1)

tk.Label(frame, text="Annual Return Rate (%):").grid(row=3, column=0)
annual_return_rate_entry = tk.Entry(frame)
annual_return_rate_entry.grid(row=3, column=1)
